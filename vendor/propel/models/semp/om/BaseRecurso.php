...
abstract class BaseRecurso extends BaseObject implements Persistent
{
    ...
    protected function doInsert(PropelPDO $con)
    {
        ...
        if ($this->isColumnModified(RecursoPeer::CO_RECURSO)) {
            $modifiedColumns[':p' . $index++]  = 'co_recurso';
        }
        if ($this->isColumnModified(RecursoPeer::NO_RECURSO)) {
            $modifiedColumns[':p' . $index++]  = 'no_recurso';
        }
        if ($this->isColumnModified(RecursoPeer::DS_RECURSO)) {
            $modifiedColumns[':p' . $index++]  = 'ds_recurso';
        }
        if ($this->isColumnModified(RecursoPeer::TP_RECURSO)) {
            $modifiedColumns[':p' . $index++]  = 'tp_recurso';
        }
        if ($this->isColumnModified(RecursoPeer::DT_CADASTRO)) {
            $modifiedColumns[':p' . $index++]  = 'dt_cadastro';
        }
        if ($this->isColumnModified(RecursoPeer::CO_USUARIO_CADASTRO)) {
            $modifiedColumns[':p' . $index++]  = 'co_usuario_cadastro';
        }
        if ($this->isColumnModified(RecursoPeer::DT_ALTERACAO)) {
            $modifiedColumns[':p' . $index++]  = 'dt_alteracao';
        }
        if ($this->isColumnModified(RecursoPeer::CO_USUARIO_ALTERACAO)) {
            $modifiedColumns[':p' . $index++]  = 'co_usuario_alteracao';
        }
        ...
        try {
            ...
            foreach ($modifiedColumns as $identifier => $columnName) {
                switch ($columnName) {
                    case 'co_recurso':
                        $stmt->bindValue($identifier, $this->co_recurso, PDO::PARAM_INT);
                        break;
                    case 'no_recurso':
                        $stmt->bindValue($identifier, $this->no_recurso, PDO::PARAM_STR);
                        break;
                    case 'ds_recurso':
                        $stmt->bindValue($identifier, $this->ds_recurso, PDO::PARAM_STR);
                        break;
                    case 'tp_recurso':
                        $stmt->bindValue($identifier, $this->tp_recurso, PDO::PARAM_INT);
                        break;
                    case 'dt_cadastro':
                        $stmt->bindValue($identifier, $this->dt_cadastro, PDO::PARAM_STR);
                        break;
                    case 'co_usuario_cadastro':
                        $stmt->bindValue($identifier, $this->co_usuario_cadastro, PDO::PARAM_INT);
                        break;
                    case 'dt_alteracao':
                        $stmt->bindValue($identifier, $this->dt_alteracao, PDO::PARAM_STR);
                        break;
                    case 'co_usuario_alteracao':
                        $stmt->bindValue($identifier, $this->co_usuario_alteracao, PDO::PARAM_INT);
                        break;
                }
            }
            $stmt->execute();
    ...
    public function setPermissaos(PropelCollection $permissaos, PropelPDO $con = null)
    {
        ...
        foreach ($permissaosToDelete as $permissaoRemoved) {
            $permissaoRemoved->setRecurso(null);
        }
